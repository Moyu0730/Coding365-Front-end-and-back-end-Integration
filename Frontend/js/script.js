    ...
    $("#book_grid").kendoGrid({
        dataSource: {
            data: {
                transport:{
                    read:{
                        url: "https://localhost:44314/api/code/bookclass",
                        dataType: "json",
                        type: "post"
                    }
                }
            }, /*Part2. TODO: 請改由remote datasource呼叫api取得資料來源 簡報某一頁*/ 
            schema: {
                model: {
                    id:"BookId",
                    fields: {
...
/*Part2. TODO: 新增/修改/查詢/刪除改為從api讀取*/
...
function addBook() { 
    ...
    $.ajax({
        type: "POST",
        url: "https://localhost:44314/api/bookmatain/addbook",
        dataType: "json",
        data: book,
...
function updateBook(bookId){
    ...
    var bookKeeperCname=
        bookKeeperId==""?"":memberData.find(m=>m.UserId==bookKeeperId).bookK;
...
function queryBook(){
    ...
    /*搜尋書籍改由用api取得資料, 請將串接參數補齊並可使用任一條件查詢*/
    $("#book_grid").kendoGrid({
        dataSource: {
            transport: {
                read:{
                    url: "https://localhost:44314/api/bookmatain/querybook",
                    dataType: "json",
                    type: "post",
                    data: function(){
                        return{
...
function deleteBook(e) {
    ...
    if (confirm("確定刪除")) {
        if(checkBookDeleteable(row.BookId)){
            ...
            $.ajax({
                type: "POST",
                url: "https://localhost:44314/api/bookmatain/deletebook",
...
function setStatusKeepRelation() { 
 ...
 /*Part1. TODO: 請將下列控制項dataSource改為從api取得*/
function registerRegularComponent(){
    // OK
        /* Note：
         *  下方的
         *    1. dataTextField: "Text" 是對應到後端輸出的 "Text"
         *    2. dataValueField: "Value" 則是對應到後端輸出的 "Value"
         */
    $("#book_class_q").kendoDropDownList({
        dataTextField: "Text", // TestCode：$("#book_class_q").data("kendoDropDownList").text()
        dataValueField: "Value", // TestCode：$("#book_class_q").data("kendoDropDownList").value()
    ...
    $("#book_class_d").kendoDropDownList({
        dataTextField: "Text", // TestCode：$("#book_class_d").data("kendoDropDownList").text()
        dataValueField: "Value", // TestCode：$("#book_class_d").data("kendoDropDownList").value()
        dataSource: {
            transport:{
                read:{
                    url: "https://localhost:44314/api/code/bookclass",
    ...
    $("#book_keeper_q").kendoDropDownList({
        dataTextField: "Text", // UserCName -> TestCode：$("#book_keeper_q").data("kendoDropDownList").text()
        dataValueField: "Value", // UserId -> TestCode：$("#book_keeper_q").data("kendoDropDownList").value()
        // dataSource: memberData,
        dataSource: {
            transport:{
                read:{
                    url: "https://localhost:44314/api/code/member",
                    dataType: "json",
                    type: "post"
                }
            }
    ...
    $("#book_keeper_d").kendoDropDownList({
        dataTextField: "Text", // UserCname -> TestCode：$("#book_keeper_d").data("kendoDropDownList").text()
        dataValueField: "Value", // UserId -> TestCode：$("#book_keeper_d").data("kendoDropDownList").value()
        // dataSource: memberData,
        dataSource: {
            transport:{
                read:{
    ...
    /* 可參考此段呼叫方式, 但此寫法並非完整請再確認 */
    // OK
    $("#book_status_q").kendoDropDownList({
        dataTextField: "Text", // StatusText -> TestCode：$("#book_keeper_q").data("kendoDropDownList").text() -> **test 一定要小寫
        dataValueField: "Value", // StatusId -> TestCode：$("#book_keeper_q").data("kendoDropDownList").value() -> **value 一定要小寫
        // dataSource: bookStatusData,
        dataSource: {
            transport:{
                read:{
                    url: "https://localhost:44314/api/code/bookstatus",
                    dataType: "json",
                    type: "post"
                }
            }
    ...
    $("#book_status_d").kendoDropDownList({
        dataTextField: "Text", // StatusText -> TestCode：$("#book_keeper_d").data("kendoDropDownList").text() -> **test 一定要小寫
        dataValueField: "Value", // StatusId -> TestCode：$("#book_keeper_d").data("kendoDropDownList").value() -> **value 一定要小寫
        // dataSource: bookStatusData,
        dataSource: {
            transport:{
                read:{
